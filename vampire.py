...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
       "*🔥 Welcome to battlefield!🔥*\n\n"
        ...
        "*💀Method = Vip user of @HIMANSHU_PAPA_BOL*" 
    )
...
async def attack(update: Update, context: CallbackContext):
    ...
    if user_id in cooldown_dict:
        ...
        if time_diff < cooldown_period:
            ...
            await context.bot.send_message(
                chat_id=chat_id,
                text=f"*⏳ You are on countdown😓 {remaining_time}*",
                parse_mode='Markdown'
            )
            return
    ...
    chat_id=chat_id,
    text=(
        f"*Sucessfully entered the battlefield🔥*\n"
        f"💢 *Attack started!* 💢\n\n"
        f"*🎯 ᴛᴀʀɢᴇᴛ ꜱᴇᴛ: {ip}:{port}*\n"
        f"*⏳ᴅᴜʀᴀᴛɪᴏɴ ʟᴏᴄᴋᴇᴅ: {duration} seconds*\n"
        f"*🔥 Method = Vip user of @HIMANSHU_PAPA_BOL*"
    ), parse_mode='Markdown')
